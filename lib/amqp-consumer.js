...
*                                         that can be awaiting acknowledgement
    ...
    async acknowledgeMessage(data) {
        await this.channel.ack(data)
            .catch(err => {
                this.logger.error({ err }, 'failed to ack message');
            });

        return _checkChannel(this, this.channel)
            .catch(err => {
                this.logger.error(
                    { err },
                    'Incorrectly acknowledged message, channel destroyed... recreating channel'
                );
    ...
    async rejectMessage(data, requeue = false) {
        const allUpTo = false;
        await this.channel.nack(data, allUpTo, requeue)
            .catch(err => {
