...
const logger = bunyan.createLogger({
    name: 'consumer', // config.get('log.name'),
...
*                                         that can be awaiting acknowledgement
    ...
    constructor(
        ...
        this.on('reconnect', this.listen.bind(this));
    }
    ...
    async assertQueue() {
        ...
        return await channel.assertQueue(queue, config);
    }
    ...
    async listen() {
        ...
        await this.assertQueue(channel, queue, exchange, exchangeType, route)
            .catch(err => {
                queueLogger.error({ err }, 'Failed to assert queue existence while listening');
                throw err;
            });
        channel.prefetch(this.prefetch);
        return channel.consume(queue, _getChannelConsumer(this, queueLogger), options)
            .then(consumed => {
                ...
                this.once('channelRecreate', () => {
                    ...
                    this.listen(queue, exchange, exchangeType, route);
                });
