...
*                                         that can be awaiting acknowledgement
     */
    constructor({ host, username, password, logger, retry, durable = false, noAck = true,
            deadLetterExchange, prefetch = 0 }) {
        super();
        const logger = logger.child({ host, username });
        this = Object.create(this, { host, username, password, logger, retry, durable, noAck, deadLetterExchange, prefetch });
    }
     ...
     */
    listen(queue, exchangeName = '', exchangeType = 'topic', route = '') {
