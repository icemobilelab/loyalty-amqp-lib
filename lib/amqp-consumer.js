import { _checkChannel, _getChannel, _getChannelConsumer, _getConnection, _logAndThrowErr, AMQP } from './amqp-base.js';
...
class AMQPConsumer extends AMQP {
   ...
   * @param { boolean } options.isDeadLetterConsumer - Will skip deadLetterExchange assert.
   */
  constructor({
    serviceName,
    host,
    ...
    // Store isDeadLetterConsumer property
    this.isDeadLetterConsumer = isDeadLetterConsumer;
  ...
  async acknowledgeMessage(data) {
    ...
    return _checkChannel(this, this._channel).catch((err) => {
      this.logger.error({ err }, 'Incorrectly acknowledged message, channel destroyed...');
   ...
   *
   * @param { object } data - This is the *exact* object emitted when a message is received
   * @param { boolean } requeue - whether to requeue the message (in the original queue) after nacking it
   *
  ...
  async rejectMessage(data, requeue = false) {
    ...
    return _checkChannel(this, this._channel).catch((err) => {
      this.logger.error({ err }, 'Incorrectly nacked message, channel destroyed... recreating channel');
      this.emit('disconnect');
    });
  }

  /**
   * Assert the queue and, if an exchange is defined, bind the queue
   * to the exchange
   *
   * @returns { Promise } if the queue and exchange definitions are correct
   */
  async assertQueue() {
    const channel = await _getChannel(this).catch(_logAndThrowErr('Failed to fetch channel to assert queue', this.logger));

    const config = {
      durable: this.durable,
      arguments: {
        'x-queue-type': 'quorum',
        'x-dead-letter-exchange': this.isDeadLetterConsumer ? undefined : this.deadLetterExchange || undefined,
      },
    };
    await channel
      .assertQueue(this.queue, config)
    ...
    if (this.deadLetterExchange) {
      ...
      await channel
        .assertExchange(this.deadLetterExchange, 'topic', { durable: true })
      ...
      await channel
        .bindQueue(deadLetterQueue, this.deadLetterExchange, '#')
        .catch(_logAndThrowErr('Failed to bind dead letter exchange to dead letter queue', this.logger));
    }

    if (this.exchange) {
      await channel
        .assertExchange(this.exchange, this.exchangeType, { durable: true })
        .catch(_logAndThrowErr('Failed to assert exchange', this.logger));
      await channel
        .bindQueue(this.queue, this.exchange, this.route)
  ...
  async listen() {
    ...
    const channel = await _getConnection(this)
      .then((connection) => {
        if (!connection) {
          _logAndThrowErr('Failed to get channel while listening', queueLogger)(new Error('Could not get a connection'));
        }
        return _getChannel(this);
      })
      .catch(_logAndThrowErr('Failed to get channel while listening', queueLogger));
    await this.assertQueue().catch(_logAndThrowErr('Failed to get channel while listening', queueLogger));
