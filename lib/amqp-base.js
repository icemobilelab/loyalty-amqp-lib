...
async function _connect(base, reconnect = false) {
    ...
    if (!(connection instanceof EventEmitter)) {
        ...
        // Clean up when process terminated or interrupted
 ...
 * **NOTE 2**: With regards to `throw_original`, throwing the original Error is `p-retry`'s default behavior.
 *
...
module.exports = { AMQP, _getChannel, _getChannelConsumer, _checkChannel, _logAndThrowErr, _getConnection };
