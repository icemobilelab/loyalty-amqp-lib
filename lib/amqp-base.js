...
class AMQP extends EventEmitter {
    ...
    constructor(options) {
        ...
        assert.ok(options.logger, 'No logger passed to AMQP lib');
        assert.ok(options.serviceName, 'Service name not passed to AMQP lib');

        assert.ok(options.host, 'AMQP host not passed to AMQP lib');
        assert.ok(options.username, 'AMQP user not passed to AMQP lib');
        assert.ok(options.password, 'AMQP password not passed to AMQP lib');

        assert.ok(options.exchange, 'No exchange passed to AMQP lib');
        assert.ok(options.exchangeType, 'Exchange type not passed to AMQP lib');
        assert.ok(options.route, 'No route passed to AMQP lib');
